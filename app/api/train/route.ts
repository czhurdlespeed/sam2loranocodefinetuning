...
export async function POST(req: NextRequest) {
  return logfire.span("Train", {}, {}, async () => {
    try {
      ...
      if (!user) {
        logfire.error("User not authorized to train models");
        return NextResponse.json(
      ...
      if (!user.approved) {
        logfire.error(`User ${session.user.name} not approved to train models`);
        return NextResponse.json(
      ...
      try {
        ...
        logfire.error("Invalid JSON in request body");
        return NextResponse.json(
      ...
      if (!rank || !checkpoint || !dataset || !epochs) {
        logfire.error("Missing required fields: rank, checkpoint, dataset, epochs");
        return NextResponse.json(
      ...
      if (typeof epochs !== "number" || epochs < 1 || epochs > 100) {
        logfire.error(`Invalid epochs value: ${epochs}`);
        return NextResponse.json(
      ...
      if (typeof rank !== "number" || ![2, 4, 8, 16, 32].includes(rank)) {
        logfire.error(`Invalid rank value: ${rank}`);
        return NextResponse.json(
      ...
      if (typeof checkpoint !== "string" || !["tiny", "small", "base_plus", "large"].includes(checkpoint)) {
        logfire.error(`Invalid checkpoint value: ${checkpoint}`);
        return NextResponse.json(
      ...
      if (typeof dataset !== "string" || !["irPOLYMER", "visPOLYMER", "TIG", "MAZAK"].includes(dataset)) {
        logfire.error(`Invalid dataset value: ${dataset}`);
        return NextResponse.json(
      ...
      logfire.info(`${session.user.name} has ${existingJobsCount.length} existing jobs`);

      const nextJobId = String(existingJobsCount.length + 1);

      logfire.info(`${session.user.name} has ${nextJobId} as the next job_id`);
